...
async function getData(url = "", keyword = "") {
  ...
  if (response.ok) {
    ...
    json.forEach((queryData, index) => {
      let tableContainer = `<h3  mt-5 style="font-weight: bold; color: black;">${getTableTitle(index)}</h3>
      `;
      tableContainer += `<table class="table table-bordered table-hover table-primary">
                          <thead>
...
function getTableTitle(index) {
  const titles = [
    "Sales Trends", 
    // "VIN and Customer Containing Transmission for Supplier",
